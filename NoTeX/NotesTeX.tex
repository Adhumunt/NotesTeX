...
\begin{document}
	...
	...
	\end{verbatim}
	If one has their own macros,\sidenote{\footnotesize Most people have their own shortcuts for commonly used mathematics, such as derivatives or integrals. For those looking for some physics shortcuts, the \textbf{supremely excellent} physics package (automatically included in \textit{Jhep for Students}) has possible everything that one can imagine.} then simply add it under this area. In \textit{Jhep for Students}, the following simplifications/shortcuts are available:
	\begin{enumerate}
